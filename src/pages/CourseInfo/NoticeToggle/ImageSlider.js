...
const ImageSlider = ({ data }) => {
  const { images, imageLabels, title, description,video,model } = data;

  return (
    <div style={{ backgroundColor: '#333', padding: '20px' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <h6 style={{ marginBottom: '1rem', color: '#fff', fontSize: '24px', textTransform: 'capitalize' }}>{title}</h6>
<p style={{ marginBottom: '2rem', color: '#ccc', fontSize: '16px', textTransform: 'capitalize' }}>{description}</p>
